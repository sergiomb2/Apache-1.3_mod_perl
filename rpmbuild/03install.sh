...
pushd $RPM_BUILD_ROOT/etc/httpd13
	...
	ln -s /usr/local/lib64/apache modules
	ln -s /var/run run
popd
...
find $RPM_BUILD_ROOT/var/www -name "*.html.html" | xargs rename.ul .html.html .html
...
pushd mod_perl-1.31
  ...
  # fix files mod
  find $RPM_BUILD_ROOT/usr/lib/perl5 -iname '*.pm' -exec chmod 0644 {} \;
...
find $RPM_BUILD_ROOT/usr/lib/perl5 -name perllocal.pod -o -name .packlist \
...
for exp in $RPM_BUILD_ROOT/usr/lib/perl5/auto/Apache/mod_perl.exp $RPM_BUILD_ROOT/usr/local/lib64/apache/httpd.exp
do
	sed -i '/^#!/ d' $exp
done
